...
\<close>
lemma cnf_linear [simp]: 
  "eval \<alpha> (of_cnf (xs @ ys)) = (eval \<alpha> (of_cnf xs) \<and> eval \<alpha> (of_cnf ys))"
...
next
  ...
  then have 1: "eval v (of_cnf (tseitin \<phi>1)) \<and> eval v (of_cnf (tseitin \<phi>2))" 
    by auto
...
fun tseitin2 :: "'a form \<Rightarrow> ('a form) cnf \<Rightarrow> ('a form) cnf"
  ...
  | "tseitin2 (Imp \<phi> \<psi>) acc
    = tseitin2 \<psi> (tseitin2 \<phi> ([(N (Imp \<phi> \<psi>)), (N \<phi>), (P \<psi>)] 
    # [(P (Imp \<phi> \<psi>)), (P \<phi>)] # [(N \<psi>), (P (Imp \<phi> \<psi>))] # acc))"
...
next
  ...
  from 2 have 8:
    "tseitin2 \<psi> (tseitin2 \<phi> ([(N (Imp \<phi> \<psi>)), (N \<phi>), (P \<psi>)] 
    # [(P (Imp \<phi> \<psi>)), (P \<phi>)] # [[(N \<psi>), (P (Imp \<phi> \<psi>))]]))
    = tseitin2 \<psi> [] @ tseitin2 \<phi> ([(N (Imp \<phi> \<psi>)), (N \<phi>), (P \<psi>)] 
    # [(P (Imp \<phi> \<psi>)), (P \<phi>)] # [[(N \<psi>), (P (Imp \<phi> \<psi>))]])" 
    by (rule allE)
  then have 9:
...
\<close>
lemma tseitin_equality: 
  "eval \<alpha> (of_cnf(tseitin \<phi>)) \<longleftrightarrow> eval \<alpha> (of_cnf(tseitin2 \<phi> []))"
proof (induction \<phi>)
  case Bot
  show ?case by auto
next
...
next
  ...
  have "eval \<alpha> (of_cnf(tseitin (Neg \<phi>))) = (eval \<alpha> (of_cnf(tseitin \<phi>)) 
    \<and> eval \<alpha> (of_cnf([(N (Neg \<phi>)), (N \<phi>)] # [[(P (Neg \<phi>)), (P \<phi>)]])))" 
    by auto
  also have "... = (eval \<alpha> (of_cnf(tseitin2 \<phi> [])) 
    \<and> eval \<alpha> (of_cnf([(N (Neg \<phi>)), (N \<phi>)] # [[(P (Neg \<phi>)), (P \<phi>)]])))" 
    unfolding IH ..
  also have "... 
    = eval \<alpha> (of_cnf(tseitin2 \<phi> [] @ [(N (Neg \<phi>)), (N \<phi>)] 
    # [[(P (Neg \<phi>)), (P \<phi>)]]))"
  ...
  also have "... = eval \<alpha> (of_cnf(tseitin2 (Neg \<phi>) []))" 
    unfolding tseitin2.simps ..
  finally show ?case .
next
  case IH: (Imp \<phi> \<psi>)
...
And we prove the symmetric case of the lemma above.
\<close>
lemma tseitin_equality2: 
  "eval \<alpha> (of_cnf(tseitin2 \<phi> [])) \<longleftrightarrow> eval \<alpha> (of_cnf(tseitin \<phi>))"
...
\<close>
lemma tseitin_eq_for_all: 
  "\<forall>\<alpha>. (eval \<alpha> (of_cnf(tseitin2 \<phi> [])) \<longleftrightarrow> eval \<alpha> (of_cnf(tseitin \<phi>)))"
proof -
...
\<close>
lemma tseitin_eq_ex:
 "(\<exists>\<alpha>. eval \<alpha> (of_cnf ([P \<phi>] # tseitin2 \<phi> []))) 
  \<longleftrightarrow> (\<exists>\<alpha>. eval \<alpha> (of_cnf ([P \<phi>] # tseitin \<phi>)))"
proof -
  ...
  proof (rule iffI)
    ...
    proof -
      from 2 obtain \<alpha> where 3: "\<alpha> \<phi>" and 4: "eval \<alpha> (of_cnf (tseitin2 \<phi> []))" 
        by auto
      from 1 have 
        "eval \<alpha> (of_cnf(tseitin2 \<phi> [])) \<longleftrightarrow> eval \<alpha> (of_cnf(tseitin \<phi>))" 
        by (rule allE)
    ...
    next
      ...
      proof -
        from 5 obtain \<alpha> where 6: "\<alpha> \<phi>" and 7: "eval \<alpha> (of_cnf (tseitin \<phi>))" 
          by auto
        from 1 have 
          "eval \<alpha> (of_cnf(tseitin2 \<phi> [])) \<longleftrightarrow> eval \<alpha> (of_cnf(tseitin \<phi>))" 
          by (rule allE)
        then have "eval \<alpha> (of_cnf(tseitin \<phi>)) \<longleftrightarrow> eval \<alpha> (of_cnf(tseitin2 \<phi> []))" 
          by (rule sym)
...
proof -
  have "sat (of_cnf ([P \<phi>] # tseitin2 \<phi> [])) 
    \<longleftrightarrow> (\<exists>\<alpha>. eval \<alpha> (of_cnf ([P \<phi>] # tseitin2 \<phi> [])))" 
    unfolding sat_def ..
  also have "... \<longleftrightarrow> (\<exists>\<alpha>. eval \<alpha> (of_cnf ([P \<phi>] # tseitin \<phi>)))" 
    unfolding tseitin_eq_ex ..
...
fun plaisted :: "bool \<Rightarrow> 'a form \<Rightarrow> ('a form) cnf"
  ...
  | "plaisted False (Imp \<phi> \<psi>) = [(P (Imp \<phi> \<psi>)), (P \<phi>)] 
    # [(N \<psi>), (P (Imp \<phi> \<psi>))] # (plaisted True \<phi> @ plaisted False \<psi>)"

text \<open>
We show two auxiliary lemmas:
\<close>
lemma [simp]: 
  "eval (eval \<alpha>) (of_cnf(plaisted True \<phi>)) 
    \<longleftrightarrow> eval (eval \<alpha>) (of_cnf(plaisted False \<phi>))"
  by (induction \<phi>) auto

lemma [simp]: 
  "eval (eval \<alpha>) (of_cnf(plaisted p \<phi>))"
proof (cases p)
  case True
...
for @{const plaisted}.
\<close>
lemma plaisted_consistent: 
    "(\<alpha> \<phi> \<and> eval \<alpha> (of_cnf(plaisted True \<phi>)) \<longrightarrow> eval (\<alpha> \<circ> Atm) \<phi>)
...
proof (rule iffI)
  ...
  show "sat \<phi>"
  proof -
    from 1 have "\<exists>\<alpha>. eval \<alpha> (of_cnf ([P \<phi>] # plaisted True \<phi>))" 
      by (simp add: sat_def)
    ...
    then have "(\<alpha> \<phi> \<and> eval \<alpha> (of_cnf(plaisted True \<phi>)) \<longrightarrow> eval (\<alpha> \<circ> Atm) \<phi>)" 
      by auto
...
next
  case IH: (Neg \<phi>)
  have "length (plaisted p (Neg \<phi>)) = 1 + length (plaisted (\<not>p) \<phi>)" 
    by (cases p) auto
  also have "... \<le> 1 + 2 * size \<phi>" using IH by auto 
  finally show ?case by auto
next
  case IH: (Imp \<phi> \<psi>)
  have "length (plaisted p (Imp \<phi> \<psi>)) 
    \<le> 2 + length (plaisted (\<not>p) \<phi>) + length (plaisted p \<psi>)"
    by (cases p) auto
...
next
  case IH: (Neg \<phi>)
  have "num_literals (plaisted p (Neg \<phi>)) = 2 + num_literals (plaisted (\<not>p) \<phi>)" 
    by (cases p) auto
...
next
  ...
  have "length (plaisted p (Imp \<phi> \<psi>)) 
    \<le> 2 + length (plaisted (\<not>p) \<phi>) + length (plaisted p \<psi>)"
    by (cases p) auto
  also have "... \<le> 2 + length (plaisted (\<not>p) \<phi>) + length (tseitin \<psi>)" 
    using IH by auto
  also have "... \<le> 2 + length (tseitin \<phi>) + length (tseitin \<psi>)" 
    using IH by auto
  finally show ?case by auto
qed
...
next
  ...
  have "num_literals (plaisted p (Imp \<phi> \<psi>)) 
          \<le> 4 + num_literals (plaisted (\<not>p) \<phi>) + num_literals (plaisted p \<psi>)"
    by (cases p) auto
  also have "... \<le> 4 + num_literals (plaisted (\<not>p) \<phi>) + num_literals (tseitin \<psi>)" 
    using IH by auto
  also have "... \<le> 4 + num_literals (tseitin \<phi>) + num_literals (tseitin \<psi>)" 
    using IH by auto
  finally show ?case by auto
qed
